...
<div class="container mx-auto p-4 h-screen">
    ...
    @endphp
    <a href="{{ route('admin.companies.internships' ,  $company ) }}" class="mb-3 inline-block bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded">Back to internshipment</a>
    <form action="{{route('admin.companies.updateinternship' , $internship )}}" method="post" class="max-w-md mx-auto grid grid-cols-2 gap-4"  enctype="multipart/form-data">
        ...
        @method('PUT')
        <div class="mb-4">
            <label for="start_date" class="block text-gray-700 text-sm font-bold mb-2">start_date</label>
            <input type="date" id="start_date" name="start_date" value="{{ $internship->start_date }}"
        ...
        <div class="mb-4">
            <label for="end_date" class="block text-gray-700 text-sm font-bold mb-2">end_date</label>
            <input type="date" id="end_date" name="end_date" value="{{ $internship->end_date }}"
                class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:ring focus:ring-blue-500">
                @error('end_date')
                    <p class="text-red-500 text-xs italic">{{ $message }}</p>
                @enderror
        ...
        <div class="mb-4 col-span-2">
            ...
            <input type="text" id="comment" name="comment"value="{{$internship->comment}}"
                class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:ring focus:ring-blue-500">
                @error('comment')
                    <p class="text-red-500 text-xs italic">{{ $message }}</p>
                @enderror
